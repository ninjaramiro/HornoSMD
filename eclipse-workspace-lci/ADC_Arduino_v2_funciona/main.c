...
// Deshabilito interrupciones PCINT
	PCICR = 0; //
	PCIFR = 0;
...
while(1){
	...
	if ((PIND & (SET<<PD6)) == 0) {
		if (modo==0){
			modo = 1; // capturando
			SET_BIT(LED_PORT, LED); //Enciendo LED
			SET_BIT(LED_PORT, LED_FRENTE); //ENCIENDO LED
	...
	if ((PIND & (SET<<PD7)) == 0) {
		modo = 0; // detenido
		CLEAR_BIT(LED_PORT, LED); //Apago LED
		CLEAR_BIT(LED_PORT, LED_FRENTE); //Apago LED
